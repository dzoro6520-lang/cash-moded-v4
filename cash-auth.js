(function () {
  ...
  function startCountdown(expiresAt) {
    ...
    if (!Number.isFinite(expiry)) {
      timeLeftBadge.textContent = 'Access time unavailable';
      timeLeftBadge.classList.remove('hidden');
      return;
    }
    ...
    tick();
    if (expiry > Date.now()) countdownInterval = setInterval(tick, 1000);
  }
  ...
  async function performLogin(username, password) {
    try {
      ...
      loginAttempts = 0;
      localStorage.removeItem('cashAppSignedOut');
      sessionStorage.removeItem('timePopupShown');
      updateAttemptsUI();
      clearLoginError();
  ...
  async function handleLogout() {
    if (window.CashSession) return window.CashSession.logout();
  ...
  function handleAuthenticated(user) {
    if (localStorage.getItem('cashAppSignedOut') === 'true') { handleUnauthenticated(); return; }
    currentUser = user;
  ...
  function handleUnauthenticated() {
    ...
    document.getElementById('m2Sheet')?.classList.remove('open');
    document.getElementById('m2Success')?.classList.add('m2-hidden');

    closeAdminPanel();
  }

  async function fetchAuthStatus() {
    if (localStorage.getItem('cashAppSignedOut') === 'true') { handleUnauthenticated(); return null; }
    try {
  ...
  bindEvents();
  window.addEventListener('pageshow', () => fetchAuthStatus());
  window.addEventListener('storage', event => {
    if (event.key === 'cashAppSignedOut' && event.newValue === 'true') handleUnauthenticated();
  });
  fetchAuthStatus();
})();
